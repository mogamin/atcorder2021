...
//     それぞれ、メンバ変数 hour, minute, second に代入する
  ...
  string format2degit(int value) {
    string ret;
    if (value<10) {
      ret = "0" + to_string(value);
    } else {
      ret = to_string(value);
    }
    return ret;
...
//     HH、MM、SSはそれぞれ時間、分、秒を2桁で表した文字列
  string to_str() {
    return format2degit(hour) + ":" + format2degit(minute) + ":" + format2degit(second);
  }
...
//     diff_second の値は -86400 ~ 86400 の範囲を取とりうる
  void shift(int diff_second) {
    ...
    if (diff_second>(60*60)) {
      diff_h = diff_second/(60*60);
      diff_second -= diff_h*(60*60);
    }

    int diff_m = 0;
    if (diff_second>60) {
      diff_m = diff_second/60;
      diff_second -= diff_m*60;
    }
